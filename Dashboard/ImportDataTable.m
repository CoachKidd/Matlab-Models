...
% Auto-generated by MATLAB on 2018/01/04 17:17:17
...
WorkItemsTable.ID = cell2mat(rawNumericColumns(:, 1));
WorkItemsTable.Link = rawStringColumns(:, 1);
WorkItemsTable.Description = rawStringColumns(:, 2);
WorkItemsTable.InBacklog = dates{:, 1};
WorkItemsTable.InDevelopment = dates{:, 2};
WorkItemsTable.DevDoneReview = dates{:, 3};
WorkItemsTable.InTest = dates{:, 4};
WorkItemsTable.TestDone = dates{:, 5};
WorkItemsTable.DeployedDev = dates{:, 6};
WorkItemsTable.SignedOff = dates{:, 7};
WorkItemsTable.Type = categorical(rawStringColumns(:, 3));
WorkItemsTable.Filter = rawStringColumns(:, 4);

% For code requiring serial dates (datenum) instead of datetime, uncomment
% the following line(s) below to return the imported dates as datenum(s).

%WorkItemsTable.InBacklog=datenum(WorkItemsTable.InBacklog);WorkItemsTable.InDevelopment=datenum(WorkItemsTable.InDevelopment);WorkItemsTable.DevDoneReview=datenum(WorkItemsTable.DevDoneReview);WorkItemsTable.InTest=datenum(WorkItemsTable.InTest);WorkItemsTable.TestDone=datenum(WorkItemsTable.TestDone);WorkItemsTable.DeployedDev=datenum(WorkItemsTable.DeployedDev);WorkItemsTable.SignedOff=datenum(WorkItemsTable.SignedOff);
